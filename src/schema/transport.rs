...
pub fn request<T: Transport, R: YaSerialize, S: YaDeserialize>(transport: &mut T, request: &R) -> Option<S> {
...
fn test_crop_xml_declaration() {
    assert_eq!(crop_xml_declaration(r#"<?xml version="1.0" encoding="utf-8"?><element />"#), "<element />");
}
